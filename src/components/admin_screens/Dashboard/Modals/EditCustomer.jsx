...
function AddNewCustomer({ customerId, userRefresh }) {
    ...
    const fetchAllUsers = async () => {
      try {
        ...
        if (customerId) {
          ...
          setLastName(response.data.customer_last_name);
          setEmail(response.data.customer_email);
          setPhoneNumber(response.data.customer_phone_number);
          setAddress(response.data.customer_address);
          setStatus(response.data.customer_status);
          setCountryId(response.data.customer_country);
        }
      } catch (error) {
        console.error(error);
      }
  ...
  return (
    <>
      ...
      <div
        ...
        aria-hidden="true"
        tabIndex="-1"
        id="header-footer-modal-preview-editCustomer"
